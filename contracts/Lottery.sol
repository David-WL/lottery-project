...
/*is Ownable*/ contract Lottery {
    // record the owner of the contract
    // will be replaced later by openzeppelin Ownable
    ...
    function bet(uint number) public payable {
        ...
        require(
            playerDetailsMapping[msg.sender].numberWagered == 0,
            "You have already betted"
        );
        // CHECK #3
        // check the range of numbers allowed for betting
        require(
            number >= 1 && number <= MAX_NUMBER_TO_BET,
            string.concat(
                "You need to bet between 1 and ",
    ...
    function announceWinners(uint winningNumber) public {
        ...
        for (uint i = 0; i < playerAddressesArray.length; i++) {
            ...
            if (
                playerDetailsMapping[playerAddress].numberWagered ==
                winningNumber
            ) {
                ...
                totalWinningWager += playerDetailsMapping[playerAddress]
                    .amountWagered;
                winnerCount++;
            }
        }
